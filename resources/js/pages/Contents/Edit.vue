...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Edit Content',
        href: `/admin/contents/${props.id}/edit`,
    },
];
...
const fetchContent = async () => {
    try {
        const response = await fetch(`/api/v1/contents/${props.id}`, {
            headers: {
        ...
        form.value = {
            ...
            body: content.versions?.[0]?.body || '',
            body_type: content.versions?.[0]?.body_type || 'html',
            summary: content.versions?.[0]?.summary || '',
            categories: content.categories?.map((c: any) => c.id) || [],
            media: content.media?.map((m: any) => m.id) || [],
        };
    } catch (error) {
...
const submitForm = async () => {
    ...
    try {
        ...
        errors.value = { general: 'An error occurred while updating the content' };
    } finally {
        loading.value = false;
    }
};
